/* ═══════════════════════════════════════════════════════════════════ *
 *  BusinessAssessment Engine                                        *
 *                                                                    *
 *  Takes a BusinessProfile and returns a BusinessAssessment with    *
 *  deterministic placeholder rules.                                 *
 *                                                                    *
 *  REPLACEMENT PATH: When switching to an AI model, replace the     *
 *  body of generateAssessment() with an API call. The function      *
 *  signature and the BusinessAssessment type remain unchanged —     *
 *  no consumer (Dashboard, Context) needs to be modified.           *
 * ═══════════════════════════════════════════════════════════════════ */
 ...
 *
 *  Rules:
 *  • Solo / Small team (≤3) + Side hustle or Full-time → Sole Proprietorship
 *  • Solo / Small team + Scale / National / International → LLP
 *  • Medium team (4-6) → LLP
 *  • Large / Enterprise (7+) → Pvt Ltd
 *  • Default fallback → Sole Proprietorship
 */
...
function determineBusinessStructure(profile: BusinessProfile): {
  ...
  for (const rule of structureRules) {
    if (rule.match(profile)) {
      const { explanation, match, ...structure } = rule;
 ...
 *
 *  Weighted heuristic based on profile completeness and signals:
 *  • Base: 25 (everyone starts somewhere)
 *  • Sales channels with GST-relevant paths: +12, otherwise +8
 *  • Growth goal ambition: up to +15
 *  • Team size: up to +12
 *  • Kitchen type clarity: up to +10
 *  • Business type: +8
 *  • Location: +5
 *  • GST requirement bonus: +5
 */
...
function computeReadinessScore(
  ...
  // Kitchen type — "not-sure" signals less readiness
  ...
  // GST bonus (being in GST scope means more serious business)
...
function determineNextBestAction(profile: BusinessProfile): NextBestAction {
  // Priority: GST (if required) → UDYAM → FSSAI
  if (needsGST(profile)) {
    return {
 ...
 *  PUBLIC API                                                        *
 *                                                                    *
 *  This is the ONLY function consumers import. When switching to an  *
 *  AI model, replace the body — the signature stays the same:        *
 *                                                                    *
 *    (profile: BusinessProfile) => BusinessAssessment                *
 *                                                                    *
 *  Async variant (for AI):                                           *
 *    (profile: BusinessProfile) => Promise<BusinessAssessment>       *
 * ═══════════════════════════════════════════════════════════════════ */
